...
export default function nce() {
  return (
    <motion.div initial={{ opacity: 0, y: 60, scale: 0.94 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ delay: 0.1, duration: 0.38, ease: [.15,.26,0,1] }} className="pt-10 items-center flex flex-col gap-1.5">
        ...
        <motion.div className="items-center flex flex-col gap-1.5" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.14, duration: 0.44, ease: [.15,.26,0,1] }}>
            <h4>콘텐츠 없음</h4>
            <motion.p initial={{ opacity: 0, y: 3 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.21, duration: 0.44, ease: [.15,.26,0,1] }}>이 페이지 내에 콘텐츠가 존재하지 않습니다.</motion.p>
        </motion.div>
    </motion.div>
  );
}
